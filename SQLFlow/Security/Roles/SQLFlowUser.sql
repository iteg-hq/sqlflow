...
-- This user will call flow.Main in order to execute the packages. From 
-- there, flow.Log would be available through ownership chaining, so 
-- until flow items need to be dispatched, EXECUTE on flow.Main would give us 
...
-- chain, so when packages come back to SQLFlow and call e.g. flow.Log,
